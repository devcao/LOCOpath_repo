...
results = Logistic.Wald.Test.Power(n = n, p = p, beta=c(1+bb,rep(1,2),rep(0, 9)), rho=rho, iter = iter, which.covariate = 1, betaNull = 1)

print(mem_used())
f1 = paste0("~/hdi_simu/results/non_log_large_sig_pc_12ttest_",p,"_", 'rho',rho,'beta_',bb,".RData")
save(results,file = f1)

}

#for (rho in list(0, 0.5, 0.9, 'weak_equl','equl')){
#results = Poisson.Wald.Test.Power(n = n, p = p, beta=c(bb,rep(1,2),rep(0, 9)), rho=rho, iter = iter, which.covariate = 1, betaNull = 0)

#print(mem_used())
#f1 = paste0("~/hdi_simu/results/poi_pc_12ttest_",p,"_", 'rho',rho,'beta_',bb,".RData")
#save(results,file = f1)
